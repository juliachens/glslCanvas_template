...
void main() {
    ...
    //亂數作用雲霧
    float fog= fbm(0.4*uv+vec2(-0.1*u_time, -0.02*u_time))*0.312+0.1;
    ...
    {
    //float index=0.0;
    //float noise_position= smoothstep(-0.2, 0., -uv.y+0.084); option 1
    float noise_position= interact;  //option 2 
    float radius_noise=noise(vec3(4.900*uv,index+u_time*0.292))*0.112*noise_position;
    float radius=0.588+radius_noise;
    ...
    //滑鼠原點 //result= 1.0-mouseEffect(uv, mouse, 0.3); 黑底白圓
...
{
    float dist=length(uv-mouse);
    return 1.2-smoothstep(size*1.668, size, dist);  //size
    //return pow(dist, 0.5);
}
...
void main() {
    ...
    //互動陰晴圓缺
    float interact=1.-mouseEffect(uv,mouse,0.310);
    ...
    float strength =(0.08*breathing+0.2);          //[0.2~0.3]         //光暈強度加上動態時間營造呼吸感
    float thickness=(0.0*breathing+0.01);          //[0.1~0.2]         //光環厚度 營造呼吸感
    float glow_circle = glow(spade_dist, strength, thickness);
    result+=glow_circle;
    }
    gl_FragColor = vec4((vec3(result+fog)),1.0);
    //gl_FragColor = vec4(vec3(circle_dist),1.0); 
    //gl_FragColor = vec4((vec3(result)+fog)*dir*vec3(0.858,0.880,1.000)*0.884,1.0);
}
*/
