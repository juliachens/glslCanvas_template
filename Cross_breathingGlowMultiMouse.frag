...
void main() {
    ...
    float dir= dot(point, (uv))+0.782;
    
    //互動陰晴圓缺
    float interact=1.024-mouseEffect(uv,mouse,0.718);
    
    //亂數作用雲霧
    float fog= fbm(0.896*uv+vec2(-0.1*u_time, -0.02*u_time))*0.5+-0.28;
    ...
    float cross_noise = noise(vec3(3.2*uv,index+u_time*0.732))*0.272*cross_position;
    float cross_dist= abs(cross(uv,1.400)+cross_noise);
    ...
    float strength =(0.08*breathing+0.280);          //[0.2~0.3]         //光暈強度加上動態時間營造呼吸感
    ...
    gl_FragColor = vec4((vec3(result)+fog)*vec3(0.925,0.269,0.190)*0.972,1.0);
}
